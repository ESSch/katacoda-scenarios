...
когда под "зависает" трафик переключается с него на другие, что-бы не создавать очередь. `let status = fs.existsSync(status.txt) == 'besy'` Нагрузка 1 запрос в секунду.
